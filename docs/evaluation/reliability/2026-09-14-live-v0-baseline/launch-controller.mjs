import { createServer } from '/Users/lbld/Documents/ChatGPT/Wememo/node_modules/vite/dist/node/index.js'
import { assessInfrastructure, CANARY_POLICY } from './canary-policy.mjs'
...
import { createEvaluationCurlTransport } from '/Users/lbld/Documents/ChatGPT/Wememo/scripts/evaluation-curl-transport.mjs'

const root = '/Users/lbld/Documents/ChatGPT/Wememo'
...
const policyPath = resolve(dirname(launcherPath), 'canary-policy.mjs')
const launcherBytes = await readFile(launcherPath)
const policyBytes = await readFile(policyPath)
const launcherHash = hash(launcherBytes); const policyHash = hash(policyBytes)
const operatorUnchanged = async () => hash(await readFile(launcherPath)) === launcherHash && hash(await readFile(policyPath)) === policyHash
...
try {
  if (process.argv.slice(2).includes('--help')) {
    ...
    metadata = { operatorControl: { policy: CANARY_POLICY, launcherSha256: launcherHash, policySha256: policyHash, productionSourcesUnmodified: true }, version: 'wememo-real-model-reliability-v2', createdAt, mode: options.mode, providerId, modelId, variants: options.variants, sets: options.sets,
    ...
    await writeFile(resolve(directory, 'launch-controller.mjs'), launcherBytes, { flag: 'wx', mode: 0o600 })
    await writeFile(resolve(directory, 'canary-policy.mjs'), policyBytes, { flag: 'wx', mode: 0o600 })
    ...
    const report = await runReliabilityEvaluation(fixtures, factory, { ...options, providerId, modelId, signal: controller.signal,
      async onCanary(rows) {
        ...
        const assessment = options.mode === 'live' ? assessInfrastructure(rows) : { stop: false, classificationValid: true, reason: null }
        const checks = { operatorControlUnchanged: await operatorUnchanged(), infrastructureAcceptable: !assessment.stop, diagnosticClassificationValid: assessment.classificationValid, completedEight: rows.length === 8, normalSix: rows.filter(row => row.set === 'normal').length === 6,
          stressTwo: rows.filter(row => row.set === 'stress').length === 2,
          persistedExactly: JSON.stringify(stored) === JSON.stringify(rows),
        ...
        await save('canary.json', { version: 'wememo-v0-canary-v1', checks, infraPass, records: rows, qualityScored: false, assessment, providerReturnedEveryTurn: rows.every(row => row.providerPass === true) })
        console.log(JSON.stringify({ stage: 'canary-review', runs: 8, infraPass, checks, assessment }))
        ...
        const approved = await operatorUnchanged() && decision.trim() === 'CONTINUE' && finalHashesMatch && !controller.signal.aborted && execFileSync('git', ['rev-parse', 'HEAD'], { cwd: root, encoding: 'utf8' }).trim() === gitHead
      ...
      async onRecord(record, review) {
        ...
        console.log(JSON.stringify({ completed: records.length, set: record.set, question: record.questionId, repeat: record.repeatIndex,
          mode: record.mode, variant: record.promptVariant, failure: record.failureCode, valid: record.endToEndPass, providerDiagnostics: record.providerDiagnostics }))
        const assessment = options.mode === 'live' ? assessInfrastructure(records) : { stop: false }
        if (assessment.stop || !await operatorUnchanged()) {
          await save('infrastructure-stop.json', { afterRun: records.length, assessment, operatorControlUnchanged: await operatorUnchanged() })
          controller.abort()
        }
      } })
    const finalSourceHashesMatch = (await Promise.all(Object.entries(sourceHashes).map(async ([path, expected]) => hash(await readFile(resolve(root, path))) === expected))).every(Boolean)
    const finalChecks = { sourceHashesMatch: finalSourceHashesMatch, operatorControlUnchanged: await operatorUnchanged(),
      gitHeadUnchanged: execFileSync('git', ['rev-parse', 'HEAD'], { cwd: root, encoding: 'utf8' }).trim() === gitHead,
      persistedExactly: JSON.stringify((await readFile(resolve(directory, 'runs.jsonl'), 'utf8')).trim().split('\n').filter(Boolean).map(line => JSON.parse(line))) === JSON.stringify(records) }
    const { reviews: items, ...metrics } = report
    await save('report.json', { ...metadata, ...metrics, finalChecks })
    ...
    else if (report.canaryStopped || !Object.values(finalChecks).every(Boolean)) process.exitCode = 2
  }
} catch {
